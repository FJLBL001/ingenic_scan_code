...
int nv_open(struct partition *parti, int flags)
{
	...
	nv_dev->nv_count = 0;
	nv_dev->nv_offset = 0;
	nv_dev->nv_size = parti->size;
	nv_dev->nv_blocksize = parti->erasesize;
...
static off_t nv_map_area(int fd, int flag)
{
	...
	tmp = -1;
	if(nv_dev->nv_ebcnt > 6) {
		printf("%s: nv_dev->nv_ebcnt %d > 6\n", __func__, nv_dev->nv_ebcnt);
		return -1;
	}
	for(i = 0; i < nv_dev->nv_ebcnt; i++) {
		offset = i * nv_dev->nv_blocksize;
		...
		if(buf[i][0] == DEFAULT_START_FLAG) {
			err = nv_read_area(fd, (char *)buf[i], 8, offset + nv_dev->nv_blocksize - 8);
			if(err < 0)
				continue;
	...
	for(i = 0; i < nv_dev->nv_ebcnt; i++) {
		if(flag == READ_FLAG) {
			tmp = (current_nv_num + i) % nv_dev->nv_ebcnt;
		} else if(flag == WRITE_FLAG) {
			tmp = (current_nv_num + i + 1) % nv_dev->nv_ebcnt;
		}
		nv_dev->nv_offset = tmp * nv_dev->nv_blocksize;
		err = nv_read_area(fd, (char *)buf[i], 4, nv_dev->nv_offset);
		if(err < 0)
			continue;
		break;
	}
	if(i == nv_dev->nv_ebcnt) {
...
ssize_t nv_read(int fd, void *buf, size_t count)
{
	...
	if(count > nv_dev->nv_blocksize) {
		printf("error:  read count 0x%x more than nv block size 0x%x\n",
			count, nv_dev->nv_blocksize);
		err = -1;
		return err;
	}
	off = lseek(fd, 0, SEEK_CUR);
	off %= nv_dev->nv_blocksize;
	if(off * count > nv_dev->nv_size) {
		printf("error:  read current location %llx plus count 0x%x more than nv max size 0x%x\n",
		       (unsigned long long )off, count, nv_dev->nv_size);
		err = -1;
		return err;
	}
	if(!nv_dev->nv_ro)
...
static int nv_map_and_write_area(int fd, const char *buf, size_t count)
{
	...
	addr = offset;
	err = erase_eraseblock(fd, addr, nv_dev->nv_blocksize);
	if (err < 0) {
	...
	*(unsigned int *)(&buf[nv_dev->nv_blocksize - 8]) = nv_dev->nv_count;
	*(unsigned int *)(&buf[nv_dev->nv_blocksize - 4]) = DEFAULT_END_FLAG;
