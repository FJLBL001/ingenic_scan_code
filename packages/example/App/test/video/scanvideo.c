...
static int read_frame(struct camera_info *camera_inf)
{
    struct v4l2_buffer buf;
    unsigned int i;
    unsigned long long starttime,endtime;
    enum io_method io = camera_inf->io_method;
    ...
    case IO_METHOD_READ:
        ...
        camera_inf->data = (__u8 *)buffers[0].start;
        camera_inf->datalen = buffers[0].length;
    ...
    case IO_METHOD_MMAP:
        ...
        camera_inf->data = buffers[buf.index].start;
        camera_inf->datalen = buffers[buf.index].length;
...
static void mainloop(struct camera_info *camera_inf)
{
    unsigned int count;
    ...
    while (count-- > 0) {
        for (;;) {
            ...
            if (read_frame(camera_inf))
                break;
        }
    }
}

void stop_capturing(struct camera_info *camera_inf)
{
...
void start_capturing(struct camera_info *camera_inf)
{
    unsigned int i;
    enum v4l2_buf_type type;
    enum io_method io = camera_inf->io_method;
...
void init_device(struct camera_info *camera_inf)
{
...
void uninit_device(struct camera_info *camera_inf)
{
    unsigned int i;
    int use_tlb = camera_inf->param.tlb;
    enum io_method io = camera_inf->io_method;
...
int process_framebuf(struct camera_info *camera_inf)
{
