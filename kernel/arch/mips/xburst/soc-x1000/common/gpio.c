...
#ifdef SUSPEND_WAKEUP_KEY_TRIGGER_EDGE
static inline void set_wakeup_key_trigger_level(struct jzgpio_chip *jz, unsigned long g, unsigned long p)
{
	unsigned long gpio, port, pin;
	gpio = SUSPEND_WAKEUP_KEY;
	port = gpio / 32;
	pin = (gpio & 0x1f);
	//printk("%d::%d, port=%d pin=%d, SUSPEND_WAKEUP_KEY= 0x%x, revert gpio irq edge() level=%d\n", i, j, port, pin, SUSPEND_WAKEUP_KEY, gpio_pin_level(jz, pin));
	if (g==port && p==pin) {
		if (SUSPEND_WAKEUP_KEY_TRIGGER_EDGE)
			writel(1<<pin, jz->reg + PXPAT0S);
		else
			writel(1<<pin, jz->reg + PXPAT0C);

		/* clear pending flag? */
		//printk("i=%d, PXFLG= 0x%08x\n", i, readl(jz->reg + PXFLG));
		writel(1<<pin, jz->reg + PXFLGC);
	}
	return;
}
#endif
...
#ifdef SUSPEND_WAKEUP_KEY_TRIGGER_EDGE
				set_wakeup_key_trigger_level(jz, i,j);
#endif
				irq = jz->irq_base + j;
