...
#define TOMBSTONE_DIR   "/tmp/tombstones"
#define TOMBSTONE_TEMPLATE (TOMBSTONE_DIR"/tombstone_%02d")

// Must match the path defined in NativeCrashListener.java
#define NCRASH_SOCKET_PATH "/tmp/system/ndebugsocket"
...
static void dump_map(log_t* log, const backtrace_map_t* map, bool fault_addr) {
  _LOG(log, logtype::MAPS, "%s%08x-%0x %c%c%c  %7d  %s\n",
...
static void dump_nearby_maps(BacktraceMap* map, log_t* log, pid_t tid) {
  ...
  if (has_fault_address && (addr < map->begin()->start)) {
    _LOG(log, logtype::MAPS, "--->Fault address falls at %08x before any mapped regions\n", addr);
  }

  BacktraceMap::const_iterator prev = map->begin();
  for (BacktraceMap::const_iterator it = map->begin(); it != map->end(); ++it) {
    if (addr >= (*prev).end && addr < (*it).start) {
      _LOG(log, logtype::MAPS, "--->Fault address falls at %08x between mapped regions\n", addr);
    }
    prev = it;
  ...
  if (has_fault_address && (addr >= (*prev).end)) {
    _LOG(log, logtype::MAPS, "--->Fault address falls at %08x after any mapped regions\n", addr);
  }
}
